...
import org.kettingpowered.ketting.utils.ArrayUtil;
...
public class KettingRedirectAdapter implements PluginTransformer {

    public static final KettingRedirectAdapter INSTANCE = new KettingRedirectAdapter();
    ...
    public static void scanMethod(byte[] bytes) {
        ...
        Ketting.LOGGER.debug(MARKER, "Scanning {}", reader.getClassName());
                    ...
                    if (METHOD_MODIFY.containsKey(key) || METHOD_REDIRECT.containsKey(key)) {
                        try {
                            ...
                            if (target != null) {
                                ...
                                if (bridge != null) {
                                    Ketting.LOGGER.debug(MARKER, "Creating bridge handler {}/{}{} to {}", node.name, method.name, method.desc, methodToString(target));
    ...
    private static void addRule(boolean modifyArgs, Class<?> owner, String name, String handlerName, Class<?>... args) {
        ...
        Method handler = methodOf(KettingReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        while (handler == null) {
            handlerArgs[0] = handlerArgs[0].getSuperclass();
            handler = methodOf(KettingReflectionHandler.class, "redirect" + capitalize(handlerName), handlerArgs);
        }
        ...
        if (modifyArgs) {
            Method modifyHandler = methodOf(KettingReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            if (modifyHandler == null) {
                handlerArgs[0] = original.getReturnType();
                modifyHandler = methodOf(KettingReflectionHandler.class, "handle" + capitalize(handlerName), handlerArgs);
            }
            if (modifyHandler == null) {
