...
import org.kettingpowered.ketting.remapper.generated.KettingReflectionHandler;
...
public class ClassLoaderRemapper extends LenientJarRemapper {
    ...
    private static final String REPLACED_NAME = Type.getInternalName(KettingReflectionHandler.class);
    ...
    public ClassLoaderRemapper(JarMapping jarMapping, JarMapping toBukkitMapping, ClassLoader classLoader) {
        ...
        this.jarMapping.setInheritanceMap(KettingRemapper.INSTANCE.inheritanceMap);
    ...
    public void tryDefineClass(String internalName) {
        ...
        Ketting.LOGGER.warn("Loading CLIENT side class: {}", internalName);
    ...
    public Map.Entry<String, String> tryClimb(Map<String, String> map, String owner, String name, int access) {
        ...
        if (mapped == null && (access == -1 || (!Modifier.isPrivate(access) && !Modifier.isStatic(access)))) {
            ...
            if (KettingRemapper.INSTANCE.inheritanceMap.hasParents(owner)) {
                parents = KettingRemapper.INSTANCE.inheritanceMap.getParents(owner);
            } else {
                parents = GlobalClassRepo.inheritanceProvider().getParents(owner);
                KettingRemapper.INSTANCE.inheritanceMap.setParents(owner, parents);
            }
    ...
    private Class<?> generateReflectionHandler() {
        try {
            ClassNode node = MixinService.getService().getBytecodeProvider().getClassNode(Type.getInternalName(KettingReflectionHandler.class));
            Preconditions.checkNotNull(node, "node");
            ClassWriter writer = new ClassWriter(0);
            String name = Type.getInternalName(KettingReflectionHandler.class) + "_" + COUNTER.getAndIncrement();
    ...
    private static class NameRemapper extends Remapper {
        ...
        private static final String ORIGIN = Type.getInternalName(KettingReflectionHandler.class);
    ...
    private static class PluginClassWriter extends ClassWriter {
        ...
        private String getSuper(final String typeName) {
            ...
            if (node == null) {
                Ketting.LOGGER.warn("Failed to find class {}", typeName);
                return "java/lang/Object";
            }
            return KettingRemapper.getNmsMapper().map(node.superName);
        }
    }
    ...
    private static byte[] dump(byte[] bytes) {
        try {
            if (KettingRemapper.DUMP != null) {
                String className = new ClassReader(bytes).getClassName() + ".class";
                int index = className.lastIndexOf('/');
                if (index != -1) {
                    File file = new File(KettingRemapper.DUMP, className.substring(0, index));
                    file.mkdirs();
                    Files.write(file.toPath().resolve(className.substring(index + 1)), bytes);
                } else {
                    Files.write(KettingRemapper.DUMP.toPath().resolve(className), bytes);
                }
            }
        } catch (Exception e) {
            Ketting.LOGGER.error("Failed to dump class " + new ClassReader(bytes).getClassName(), e);
        }
        return bytes;
    }
}
