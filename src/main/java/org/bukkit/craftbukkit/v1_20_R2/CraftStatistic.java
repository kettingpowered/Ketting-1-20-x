...
public enum CraftStatistic {
    ...
    static {
        Builder statisticBuilder = ImmutableBiMap.builder();
        CraftStatistic[] acraftstatistic;
        int i = (acraftstatistic = values()).length;

        for (int j = 0; j < i; ++j) {
            CraftStatistic statistic = acraftstatistic[j];
    ...
    public static Statistic getBukkitStatistic(Stat statistic) {
        ...
        return (Statistic) CraftStatistic.statistics.get(nmsKey);
    }
    ...
    public static Stat getNMSStatistic(Statistic bukkit) {
        ...
        Stat nms = Stats.CUSTOM.get((ResourceLocation) CraftStatistic.statistics.inverse().get(bukkit));
    ...
    public static Stat getMaterialStatistic(Statistic stat, Material material) {
        try {
            return stat == Statistic.MINE_BLOCK ? Stats.BLOCK_MINED.get(CraftMagicNumbers.getBlock(material)) : (stat == Statistic.CRAFT_ITEM ? Stats.ITEM_CRAFTED.get(CraftMagicNumbers.getItem(material)) : (stat == Statistic.USE_ITEM ? Stats.ITEM_USED.get(CraftMagicNumbers.getItem(material)) : (stat == Statistic.BREAK_ITEM ? Stats.ITEM_BROKEN.get(CraftMagicNumbers.getItem(material)) : (stat == Statistic.PICKUP ? Stats.ITEM_PICKED_UP.get(CraftMagicNumbers.getItem(material)) : (stat == Statistic.DROP ? Stats.ITEM_DROPPED.get(CraftMagicNumbers.getItem(material)) : null)))));
    ...
    public static EntityType getEntityTypeFromStatistic(Stat statistic) {
        Preconditions.checkArgument(statistic != null, "NMS Statistic cannot be null");
    ...
    public static Material getMaterialFromStatistic(Stat statistic) {
        Object object = statistic.getValue();
        Item statisticItemValue;

        if (object instanceof Item && (statisticItemValue = (Item) object) == (Item) object) {
            return CraftMagicNumbers.getMaterial(statisticItemValue);
        } else {
            Object object1 = statistic.getValue();
            Block statisticBlockValue;

            return object1 instanceof Block && (statisticBlockValue = (Block) object1) == (Block) object1 ? CraftMagicNumbers.getMaterial(statisticBlockValue) : null;
        }
    }
    ...
    public static void setStatistic(ServerStatsCounter manager, Statistic statistic, int newValue, ServerPlayer player) {
        ...
        if (player != null) {
            player.level().getCraftServer().getScoreboardManager().getScoreboardScores(nmsStatistic, player.getScoreboardName(), (scorex) -> {
                scorex.setScore(newValue);
            });
        }
