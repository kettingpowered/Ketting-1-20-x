...
public class CraftPotionUtil {
    ...
    private static final BiMap extendable = ImmutableBiMap.builder().put(PotionType.NIGHT_VISION, PotionType.LONG_NIGHT_VISION).put(PotionType.INVISIBILITY, PotionType.LONG_INVISIBILITY).put(PotionType.JUMP, PotionType.LONG_LEAPING).put(PotionType.FIRE_RESISTANCE, PotionType.LONG_FIRE_RESISTANCE).put(PotionType.SPEED, PotionType.LONG_SWIFTNESS).put(PotionType.SLOWNESS, PotionType.LONG_SLOWNESS).put(PotionType.WATER_BREATHING, PotionType.LONG_WATER_BREATHING).put(PotionType.POISON, PotionType.LONG_POISON).put(PotionType.REGEN, PotionType.LONG_REGENERATION).put(PotionType.STRENGTH, PotionType.LONG_STRENGTH).put(PotionType.WEAKNESS, PotionType.LONG_WEAKNESS).put(PotionType.TURTLE_MASTER, PotionType.LONG_TURTLE_MASTER).put(PotionType.SLOW_FALLING, PotionType.LONG_SLOW_FALLING).build();

    public static PotionType fromBukkit(PotionData data) {
        PotionType type;

        if (data.isUpgraded()) {
            type = (PotionType) CraftPotionUtil.upgradeable.get(data.getType());
        } else if (data.isExtended()) {
            type = (PotionType) CraftPotionUtil.extendable.get(data.getType());
        } else {
            type = data.getType();
        }
    ...
    public static PotionData toBukkit(PotionType type) {
        if (type == null) {
            ...
            PotionType potionType = (PotionType) CraftPotionUtil.extendable.inverse().get(type);

            if (potionType != null) {
                return new PotionData(potionType, true, false);
            } else {
                potionType = (PotionType) CraftPotionUtil.upgradeable.inverse().get(type);
