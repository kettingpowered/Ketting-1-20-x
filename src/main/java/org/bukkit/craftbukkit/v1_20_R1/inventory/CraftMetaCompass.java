...
public class CraftMetaCompass extends CraftMetaItem implements CompassMeta {
    ...
    public Location getLodestone() {
        ...
        Optional<ResourceKey<Level>> key = Level.RESOURCE_KEY_CODEC.parse(NbtOps.INSTANCE, lodestoneWorld).result();
    ...
    public void setLodestone(Location lodestone) {
        ...
        if (lodestone == null) {
            ...
            ResourceKey<Level> key = ((CraftWorld) lodestone.getWorld()).getHandle().dimension();
            DataResult<Tag> dataresult = Level.RESOURCE_KEY_CODEC.encodeStart(NbtOps.INSTANCE, key);
