...
public class CraftWorld extends CraftRegionAccessor implements World {
    ...
    public double getHumidity(int x, int y, int z) {
        return this.world.getNoiseBiome(x >> 2, y >> 2, z >> 2).value().getClimateSettings().downfall(); //Ketting
    }
    ...
    public void setDifficulty(Difficulty difficulty) {
        ((net.minecraft.world.level.storage.PrimaryLevelData) this.getHandle().serverLevelData).setDifficulty(net.minecraft.world.Difficulty.byId(difficulty.getValue()));
    }
    ...
    public boolean canGenerateStructures() {
        return ((net.minecraft.world.level.storage.PrimaryLevelData) world.serverLevelData).worldGenOptions().generateStructures();
    }
    ...
    public void playSound(Entity entity, Sound sound, org.bukkit.SoundCategory category, float volume, float pitch) {
        ...
        ClientboundSoundEntityPacket packet = new ClientboundSoundEntityPacket(BuiltInRegistries.SOUND_EVENT.wrapAsHolder(CraftSound.getSoundEffect(sound)), SoundSource.valueOf(category.name()), craftEntity.getHandle(), volume, pitch, getHandle().getRandom().nextLong());
    ...
    public void playSound(Entity entity, String sound, org.bukkit.SoundCategory category, float volume, float pitch) {
        ...
        ClientboundSoundEntityPacket packet = new ClientboundSoundEntityPacket(Holder.direct(SoundEvent.createVariableRangeEvent(new ResourceLocation(sound))), SoundSource.valueOf(category.name()), craftEntity.getHandle(), volume, pitch, getHandle().getRandom().nextLong());
        ChunkMap.TrackedEntity entityTracker = getHandle().getChunkSource().chunkMap.entityMap.get(entity.getEntityId());
        if (entityTracker != null) {
            entityTracker.broadcastAndSend(packet);
        }
